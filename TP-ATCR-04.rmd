...
Les calculs sont donc indentiques à la question 1, à part l'ajout de la PSV et du SRG. En effet, l'un des critères pour recevoir la SRG est de recevoir la PSV. Cette dernière nécessite d'avoir 65 ans. (On constate à partir des informations données sur Virginie qu'elle remplit tous les autres critères d'admissibilité à la PSV). Dans son cas, elle est également admissible au SRG, car son revenu est considéré comme faible. Les montants pour ces deux rentes sont fixes, mais le montant du SRG est susceptible d'être réduit et les calculs suivants montrent que c'est d'ailleurs le cas. La réduction qui sera appliquée au montant reçu de SRG est de 1\$ pour chaque 2\$ de revenu gagné et cette réduction sera appliquée au montant de la rente maximale pour un célibataire pouvant recevoir la SRG (10779,84 \$ en 2019. Dans notre cas, ce montant devra être indexé sur 6 ans.).  Pour notre cas, les seuls revenus affectant le montant du SRG sont les rentes de la RRQ et le calcul est comme suit:
$$
rente.SRG = RenteMaxSRG.indexée-\frac{(rente.RRBindexée+RenteS1indéxée)-3500}{2}
$$
Dans notre situation, le montant de la SRG sera donc de 9857\$. De plus, le montant de rente qu'elle recevra de la PSV indexée est de 8493\$. Pour obtenir ce montant, il suffit d'indexer le montant fixe de 2019 sur 6 ans. Le calcul est le même que celui effectué plus tôt. La variable "t" représente cette fois le nombre d'années écoulées depuis l'année correspondant au montant fixe utilisé (2019).
 
Virginie aura donc un remplacement de revenu de retraite de 58%. Le calcul de remplacement de revenu sera le même qu'à la question 1 excepté qu'on devra ajouter au numérateur le montant de la PSV et de la SRG.
...
Pour le calcul de la rente de la SRG, le calcul reste le même que la question 2. La rente de la SRG sera de 7236\$ suite au retrait du 1\$ de rente par 2\$ de revenu.
